#!/usr/bin/env ruby
# redis_monitor_web_polling.rb - Simple web-based Redis pub/sub message monitor using polling
...
class RedisMonitorSimple < Sinatra::Base
  ...
  post '/api/filters' do
    ...
    begin
      ...
      settings.message_filters[:message_types] = Set.new(data['message_types']) if data['message_types']

      # Update service filters
      settings.message_filters[:services] = Set.new(data['services']) if data['services']
    ...
    Thread.new do
      ...
      settings.subscriber.psubscribe('Messages::*') do |on|
        on.pmessage do |_pattern, channel, message|
          ...
          from_service = parsed_message.dig('_sm_header',
                                            'from') || parsed_message.dig('headers', 'from') || 'unknown'
          to_service = parsed_message.dig('_sm_header',
                                          'to') || parsed_message.dig('headers', 'to') || 'broadcast'
  ...
  def self.add_to_buffer(message)
    settings.buffer_lock.synchronize do
      ...
      settings.message_buffer.shift if settings.message_buffer.size > settings.buffer_size
    end
  end


  # Normalize service names from Redis messages to match filter options
  def self.normalize_service_name(service_name)
    return service_name if service_name.nil? || service_name.empty?

    case service_name.downcase
    when /^citizen/, 'citizens'
      'citizens'
    when '911', /dispatch/, /emergency.*dispatch/
      'emergency-dispatch-center'
    when /police/
      'police-department'
    when /fire/
  ...
  def self.should_display_message?(message_type, from_service, to_service)
    ...
    # Normalize service names for filtering
    normalized_from = normalize_service_name(from_service)
    normalized_to = normalize_service_name(to_service)
    ...
    service_match = message_filters[:services].empty? ||
                    message_filters[:services].include?(normalized_from) ||
                    message_filters[:services].include?(normalized_to) ||

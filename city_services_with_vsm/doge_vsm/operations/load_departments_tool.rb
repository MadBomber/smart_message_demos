  ...
  class Operations < VSM::Operations
    class LoadDepartmentsTool < VSM::ToolCapsule
      ...
      tool_schema({
                    ...
                    properties: {
                      pattern: { type: 'string',
                                 description: 'Glob pattern for YAML files (default: *_department.yml)' },
                      validate: { type: 'boolean',
                                  description: 'Validate against template structure (default: true)' },
                      template_path: { type: 'string',
                                       description: 'Path to template file (default: generic_department.yml.example)' }
                    }
                  })
      ...
      def run(args)
        ...
        if validate && validation_results.any?
          result[:validation_summary] = {
            ...
            average_completeness: validation_results.map do |v|
              v[:completeness_score] || 0
            end.sum.to_f / validation_results.length
          }
        end
      ...
      def extract_keywords(config)
        ...
        if config['routing_rules']
          config['routing_rules'].each do |_, rules|
            Array(rules).each do |rule|
              keywords.merge(rule['keywords'].map(&:downcase)) if rule['keywords']
            end
          end
        end
        ...
        if config['action_configs']
          config['action_configs'].each do |_, action_config|
            next unless action_config['response_template']
      ...
      def validate_against_template(config, template_path, file_path)
        ...
        required_sections = %w[department capabilities message_types routing_rules message_actions
                               action_configs logging]
        ...
        required_sections.each do |section|
          errors << "Missing required section: #{section}" unless config.key?(section)
        end

        # Check department fields
        if config['department']
          department_required.each do |field|
            errors << "Missing required department field: #{field}" unless config['department'].key?(field)
          end
        else
        ...
        total_sections = required_sections.length + %w[ai_analysis resources integrations
                                                       custom_settings].length
        present_sections = (required_sections + %w[ai_analysis resources integrations
                                                   custom_settings]).count do |section|
          config.key?(section)
        end
